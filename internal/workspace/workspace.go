...
func (w *Workspace) Info() experience.Info {
	if w.desktop {
		return experience.Info{ID: "worldr.workspace", Title: "worldr — Spatial workspace", Controls: "use the right app rail to open native tools · drag the bottom-right scene rotation pad to orbit · drag a window grip or the photo itself; release moving to throw, grab again to stop · Super+drag also moves windows where the host permits · scroll while dragging for depth · New Terminal or Ctrl+Alt+Enter opens a shell · Ctrl+Alt+G opens spatial portals · Ctrl+Alt+Left/Right jumps groups · Ctrl+Alt+O finds windows · Enter reads the selected app · P changes presentation · F1 opens Help"}
	}
