...
func TestHandleOverviewKeysF12AndEsc(t *testing.T) {
	...
	now := time.Unix(1, 0)
	handleOverviewKeys(&ov, ptr, scene, now, 800, 600, &meta)
	if !ov.Want {
		t.Fatal("F12 should open")
	}
	ptr = &input.Pointer{Quit: true, Keys: []input.Key{{Code: keyEsc, Pressed: true}}}
	handleOverviewKeys(&ov, ptr, scene, now.Add(time.Second), 800, 600, &meta)
	if ov.Want {
...
func TestHandleOverviewKeysSuperTabAndEnter(t *testing.T) {
	...
	handleOverviewKeys(&ov, ptr, scene, now.Add(time.Millisecond), 800, 600, &meta)
	...
	handleOverviewKeys(&ov, ptr, scene, now.Add(2*time.Millisecond), 800, 600, &meta)
	if ov.Want {
...
func TestHandleOverviewKeysF12WaylandOffset(t *testing.T) {
	...
	handleOverviewKeys(&ov, ptr, scene, time.Unix(1, 0), 800, 600, &meta)
	if !ov.Want {
