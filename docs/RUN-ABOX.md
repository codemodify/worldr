...
| `brave` / Chromium | ozone Wayland + dmabuf | **Try (0.9.26)** | 0.9.25 SEGVd in Qt Wayland shim (`CreateQtInterface` → `wl_display_roundtrip`) on `wl_data_device.selection(nil)` during platform init (same as ark). 0.9.26 defers `selection` until `wl_keyboard.enter`. GPU path still 0.9.21 (`--disable-gpu` if that dies). |
| `ark` | Qt6 Wayland | **Try (0.9.26)** | 0.9.25: `WAYLAND_DEBUG=1` last event was `wl_data_device#10.selection(nil)`, then SIGSEGV in `libQt6WaylandClient`. Second sync must now reach `callback.done`; `selection` only after `wl_keyboard.enter`. Window should open. |
...
- Brave / ark (**0.9.26**): do **not** send `wl_data_device.selection` / primary `selection` on `get_data_device` — Qt6 `createPlatformIntegration` crashes if that event arrives on the init `roundtrip` (`platformIntegration()` is still null). Send it on `wl_keyboard.enter` (and later selection changes while focused). `WAYLAND_DEBUG=1 ark`: after the second `sync`, expect `callback.done` and **no** `data_device.selection` before `keyboard.enter`; after `get_keyboard` expect `keymap` + `repeat_info`. GPU/ozone extras remain 0.9.21 (`--disable-gpu` if that path still dies). GNOME Disks still deferred (GTK).
