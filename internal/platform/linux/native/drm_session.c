...
struct worldr_drm {
  ...
  int planes_only;
  int managed;
  uint32_t requested_connector;
};

static void drop_master(worldr_drm *d) {
  if (!d->managed)
    drmDropMaster(d->fd);
}
...
static int open_card(const char *card, char *picked, size_t picked_len,
                     char *err, int errlen) {
  if (card && card[0]) {
  ...
  seterr(err, errlen,
         "no /dev/dri/cardN (no GPU, or missing video/render group)", ENOENT);
  return -1;
}

static int take_managed_fd(worldr_drm *d, int fd, char *err, int errlen) {
  d->managed = 1;
  d->fd = fcntl(fd, F_DUPFD_CLOEXEC, 0);
  if (d->fd < 0) {
    seterr(err, errlen, "duplicate managed DRM descriptor", errno);
    return -1;
  }
  char path[64];
  snprintf(path, sizeof(path), "/proc/self/fd/%d", d->fd);
  ssize_t n = readlink(path, d->card, sizeof(d->card) - 1);
  if (n > 0)
    d->card[n] = 0;
  return 0;
}

static int drm_create_buffer(const char *card, int managed_fd,
                             uint32_t connector_id, worldr_drm **out, char *err,
                             int errlen) {
  ...
  d->requested_connector = connector_id;
  if (managed_fd >= 0) {
    if (take_managed_fd(d, managed_fd, err, errlen) != 0) {
      free(d);
      return -1;
    }
  } else if (card && card[0]) {
    ...
    if (drmSetMaster(d->fd) != 0) {
      seterr(err, errlen,
             "drmSetMaster failed — another compositor owns this card. Spare "
             "VT: Ctrl+Alt+F3 + scripts/try-tty.sh",
             errno);
      close(d->fd);
      free(d);
      return -1;
    ...
    if (got < 0) {
      seterr(err, errlen,
             "no DRM card we could drmSetMaster — another compositor owns the "
             "GPU, or no /dev/dri/cardN. Spare VT: Ctrl+Alt+F3 + "
             "scripts/try-tty.sh",
             EBUSY);
      free(d);
      return -1;
  ...
  for (int i = 0; i < res->count_connectors; i++) {
    ...
    if (c->connection == DRM_MODE_CONNECTED && c->count_modes > 0 &&
        (!d->requested_connector ||
         c->connector_id == d->requested_connector)) {
      conn = c;
      break;
    }
  ...
  d->map = mmap(NULL, d->size, PROT_READ | PROT_WRITE, MAP_SHARED, d->fd,
                (off_t)map.offset);
  if (d->map == MAP_FAILED) {
    d->map = NULL;
  ...
  if (drmModeSetCrtc(d->fd, d->crtc_id, d->fb_id, 0, 0, &d->connector_id, 1,
                     &d->mode) != 0) {
...
int worldr_drm_create(const char *card, worldr_drm **out, char *err,
                      int errlen) {
  return drm_create_buffer(card, -1, 0, out, err, errlen);
}

static int drm_open_master(worldr_drm *d, const char *card, char *err,
                           int errlen) {
  if (card && card[0]) {
    d->fd = open_card(card, d->card, sizeof(d->card), err, errlen);
    if (d->fd < 0) {
  ...
  seterr(err, errlen,
         "no DRM card we could drmSetMaster — another compositor owns the GPU, "
         "or no /dev/dri/cardN. Spare VT: Ctrl+Alt+F3 + scripts/try-tty.sh",
         EBUSY);
  return -1;
}
...
static int drm_create_planes(const char *card, int managed_fd,
                             uint32_t connector_id, worldr_drm **out, char *err,
                             int errlen) {
  worldr_drm *d = (worldr_drm *)calloc(1, sizeof(*d));
  if (!d) {
  ...
  d->planes_only = 1;
  d->requested_connector = connector_id;
  if ((managed_fd >= 0 ? take_managed_fd(d, managed_fd, err, errlen)
                       : drm_open_master(d, card, err, errlen)) != 0) {
    free(d);
    return -1;
  }
...
int worldr_drm_create_planes(const char *card, worldr_drm **out, char *err,
                             int errlen) {
  return drm_create_planes(card, -1, 0, out, err, errlen);
}
int worldr_drm_create_fd(int fd, uint32_t connector_id, int planes_only,
                         worldr_drm **out, char *err, int errlen) {
  if (fd < 0) {
    seterr(err, errlen, "invalid managed DRM descriptor", 0);
    return -1;
  }
  return planes_only
             ? drm_create_planes(NULL, fd, connector_id, out, err, errlen)
             : drm_create_buffer(NULL, fd, connector_id, out, err, errlen);
}
int worldr_drm_outputs(int fd, worldr_drm_output *out, int capacity, char *err,
                       int errlen) {
  drmModeRes *res = drmModeGetResources(fd);
  if (!res) {
    seterr(err, errlen, "DRM connector inventory", errno);
    return -1;
  }
  if (res->count_connectors > capacity) {
    drmModeFreeResources(res);
    seterr(err, errlen, "too many DRM connectors", 0);
    return -1;
  }
  int count = 0;
  for (int i = 0; i < res->count_connectors; i++) {
    ...
    if (!c)
      continue;
    worldr_drm_output *o = &out[count++];
    memset(o, 0, sizeof(*o));
    o->id = c->connector_id;
    o->connected = c->connection == DRM_MODE_CONNECTED;
    const char *type = drmModeGetConnectorTypeName(c->connector_type);
    snprintf(o->name, sizeof(o->name), "%s-%u", type ? type : "Unknown",
             c->connector_type_id);
    if (c->count_modes > 0) {
      int mode = 0;
      for (int j = 0; j < c->count_modes; j++)
        if (c->modes[j].type & DRM_MODE_TYPE_PREFERRED) {
          mode = j;
          break;
        }
      o->width = c->modes[mode].hdisplay;
      o->height = c->modes[mode].vdisplay;
      drmModeModeInfo *m = &c->modes[mode];
      uint64_t rate = (uint64_t)m->clock * 1000000,
               divisor = (uint64_t)m->htotal * m->vtotal;
      ...
      if (m->vscan > 1)
        divisor *= m->vscan;
      o->refresh_millihz = divisor ? (uint32_t)((rate + divisor / 2) / divisor)
                                   : m->vrefresh * 1000;
    }
    drmModeFreeConnector(c);
  }
...
void worldr_drm_destroy(worldr_drm *d) {
  ...
  if (d->saved && d->fd >= 0) {
    drmModeSetCrtc(d->fd, d->saved->crtc_id, d->saved->buffer_id, d->saved->x,
                   d->saved->y, &d->connector_id, 1, &d->saved->mode);
    drmModeFreeCrtc(d->saved);
  }
  if (d->map && d->size) {
    munmap(d->map, d->size);
  ...
  if (d->overlay_fb_id && d->fd >= 0) {
    drmModeRmFB(d->fd, d->overlay_fb_id);
    d->overlay_fb_id = 0;
  }
  if (d->overlay_handle && d->fd >= 0) {
    struct drm_gem_close cl = {.handle = d->overlay_handle};
    drmIoctl(d->fd, DRM_IOCTL_GEM_CLOSE, &cl);
    d->overlay_handle = 0;
  }
  if (d->cursor_map && d->cursor_size) {
    munmap(d->cursor_map, d->cursor_size);
    d->cursor_map = NULL;
  }
  if (d->cursor_fb_id && d->fd >= 0) {
    drmModeRmFB(d->fd, d->cursor_fb_id);
    d->cursor_fb_id = 0;
  }
  if (d->cursor_handle && d->fd >= 0) {
    struct drm_mode_destroy_dumb destroy = {.handle = d->cursor_handle};
    drmIoctl(d->fd, DRM_IOCTL_MODE_DESTROY_DUMB, &destroy);
    d->cursor_handle = 0;
  }
  if (d->fb_id) {
...
const char *worldr_drm_card(const worldr_drm *d) { return d ? d->card : ""; }

int worldr_drm_fd(const worldr_drm *d) { return d ? d->fd : -1; }

uint32_t worldr_drm_connector_id(const worldr_drm *d) {
  return d ? d->connector_id : 0;
}

int worldr_drm_planes_only(const worldr_drm *d) { return d && d->planes_only; }
...
uint32_t worldr_drm_stride(const worldr_drm *d) { return d ? d->pitch : 0; }
...
static int atomic_set_primary(worldr_drm *d, uint32_t fb_id) {
  ...
  for (uint32_t i = 0; i < pres->count_planes; i++) {
    ...
    if (pl->crtc_id == d->crtc_id || (on_crtc && !plane_id)) {
      uint32_t type =
          prop_id(d->fd, pl->plane_id, DRM_MODE_OBJECT_PLANE, "type");
      drmModeObjectProperties *op = drmModeObjectGetProperties(
          d->fd, pl->plane_id, DRM_MODE_OBJECT_PLANE);
      int is_primary = 0;
      if (op && type) {
        for (uint32_t k = 0; k < op->count_props; k++) {
          if (op->props[k] == type &&
              op->prop_values[k] == DRM_PLANE_TYPE_PRIMARY) {
            is_primary = 1;
          }
        }
      }
...
int worldr_drm_scanout_dmabuf(worldr_drm *d, int dmabuf_fd, uint32_t width,
  ...
  if (d->planes_only) {
    seterr(err, errlen,
           "planes-only DRM (vk-display sidecar); no primary scanout", 0);
    return -1;
  }
  ...
  uint32_t fb_id = 0;
  int added = -1;
  int use_mod = modifier != 0 && modifier != DRM_FORMAT_MOD_LINEAR &&
                modifier != DRM_FORMAT_MOD_INVALID;
  if (use_mod) {
    added = drmModeAddFB2WithModifiers(d->fd, width, height, fourcc, handles,
                                       pitches, offsets, mods, &fb_id,
                                       DRM_MODE_FB_MODIFIERS);
  }
  if (added != 0) {
    added = drmModeAddFB2(d->fd, width, height, fourcc, handles, pitches,
                          offsets, &fb_id, 0);
  }
  if (added != 0) {
  ...
  if (committed != 0) {
    committed = drmModeSetCrtc(d->fd, d->crtc_id, fb_id, 0, 0, &d->connector_id,
                               1, &d->mode);
  }
  if (committed != 0) {
...
int worldr_drm_scanout_restore(worldr_drm *d, char *err, int errlen) {
  ...
  if (r != 0) {
    r = drmModeSetCrtc(d->fd, d->crtc_id, d->fb_id, 0, 0, &d->connector_id, 1,
                       &d->mode);
  }
  drop_scan_fb(d);
  d->scan_active = 0;
...
static uint32_t plane_type_val(int fd, uint32_t plane_id) {
  uint32_t type = prop_id(fd, plane_id, DRM_MODE_OBJECT_PLANE, "type");
  if (!type) {
    return (uint32_t)-1;
  }
...
static void discover_planes(worldr_drm *d) {
  if (!d || d->fd < 0 || d->planes_probed) {
    return;
  }
  d->planes_probed = 1;
  d->cursor_max_w = 256;
  ...
  for (uint32_t i = 0; i < pres->count_planes; i++) {
    ...
    if (!(pl->possible_crtcs & d->crtc_mask)) {
      drmModeFreePlane(pl);
      continue;
    }
    uint32_t typ = plane_type_val(d->fd, pl->plane_id);
    if (typ == DRM_PLANE_TYPE_OVERLAY && !d->overlay_id) {
      d->overlay_id = pl->plane_id;
    } else if (typ == DRM_PLANE_TYPE_CURSOR && !d->cursor_id) {
      d->cursor_id = pl->plane_id;
    }
    drmModeFreePlane(pl);
  }
  drmModeFreePlaneResources(pres);
...
int worldr_drm_plane_caps(worldr_drm *d, int *overlay, int *cursor,
  ...
  discover_planes(d);
  if (overlay) {
    *overlay = d->overlay_id ? 1 : 0;
  }
  if (cursor) {
    *cursor = d->cursor_id ? 1 : 0;
  }
  if (cursor_w) {
    *cursor_w = d->cursor_max_w;
  }
  if (cursor_h) {
    *cursor_h = d->cursor_max_h;
  }
  return 0;
}

static int atomic_set_plane(worldr_drm *d, uint32_t plane_id, uint32_t fb_id,
                            int32_t x, int32_t y, uint32_t crtc_w,
                            uint32_t crtc_h, uint32_t src_w, uint32_t src_h) {
  if (!d || !plane_id) {
    return -1;
  }
  if (drmSetClientCap(d->fd, DRM_CLIENT_CAP_ATOMIC, 1) != 0) {
  ...
  drmModeAtomicAddProperty(req, plane_id, p_fb, fb_id);
  drmModeAtomicAddProperty(req, plane_id, p_crtc, fb_id ? d->crtc_id : 0);
  if (fb_id) {
    if (p_sx) {
      drmModeAtomicAddProperty(req, plane_id, p_sx, 0);
    }
    ...
    if (p_sw) {
      drmModeAtomicAddProperty(req, plane_id, p_sw, (uint64_t)src_w << 16);
    }
    if (p_sh) {
      drmModeAtomicAddProperty(req, plane_id, p_sh, (uint64_t)src_h << 16);
    }
    if (p_cx) {
      drmModeAtomicAddProperty(req, plane_id, p_cx, (uint64_t)(uint32_t)x);
    }
    if (p_cy) {
      drmModeAtomicAddProperty(req, plane_id, p_cy, (uint64_t)(uint32_t)y);
    }
    if (p_cw) {
      drmModeAtomicAddProperty(req, plane_id, p_cw, crtc_w);
    }
    if (p_ch) {
      drmModeAtomicAddProperty(req, plane_id, p_ch, crtc_h);
    }
  }
  int r = drmModeAtomicCommit(d->fd, req, 0, NULL);
  drmModeAtomicFree(req);
  return r;
}

static void drop_overlay_fb(worldr_drm *d) {
  if (!d || d->fd < 0) {
    return;
  }
  if (d->overlay_fb_id) {
...
int worldr_drm_overlay_dmabuf(worldr_drm *d, int dmabuf_fd, uint32_t width,
                              ...
                              int errlen) {
  if (!d || d->fd < 0 || dmabuf_fd < 0 || width == 0 || height == 0) {
    seterr(err, errlen, "drm overlay: missing session or dmabuf", 0);
    return -1;
  }
  discover_planes(d);
  if (!d->overlay_id) {
    seterr(err, errlen, "drm overlay: no overlay plane", 0);
    return -1;
  }
  if (pitch == 0) {
    pitch = width * 4;
  ...
  uint32_t handle = 0;
  if (drmPrimeFDToHandle(d->fd, dmabuf_fd, &handle) != 0) {
    seterr(err, errlen, "drmPrimeFDToHandle overlay", errno);
    return -1;
  }
  uint32_t handles[4] = {handle, 0, 0, 0};
  uint32_t pitches[4] = {pitch, 0, 0, 0};
  ...
  if (added != 0) {
    struct drm_gem_close cl = {.handle = handle};
    drmIoctl(d->fd, DRM_IOCTL_GEM_CLOSE, &cl);
    seterr(err, errlen, "drmModeAddFB2 overlay", errno);
    return -1;
  }
  if (atomic_set_plane(d, d->overlay_id, fb_id, x, y, width, height, width,
                       height) != 0) {
    drmModeRmFB(d->fd, fb_id);
    struct drm_gem_close cl = {.handle = handle};
    drmIoctl(d->fd, DRM_IOCTL_GEM_CLOSE, &cl);
    seterr(err, errlen, "overlay atomic commit", errno);
    return -1;
  }
  drop_overlay_fb(d);
  d->overlay_fb_id = fb_id;
  d->overlay_handle = handle;
  d->overlay_active = 1;
  return 0;
}
...
int worldr_drm_overlay_disable(worldr_drm *d, char *err, int errlen) {
  if (!d || d->fd < 0) {
    seterr(err, errlen, "drm overlay disable: missing session", 0);
    return -1;
  }
  if (!d->overlay_active) {
    return 0;
  ...
  discover_planes(d);
  if (d->overlay_id) {
    (void)atomic_set_plane(d, d->overlay_id, 0, 0, 0, 0, 0, 0, 0);
  }
  drop_overlay_fb(d);
  d->overlay_active = 0;
  return 0;
...
static int ensure_cursor_fb(worldr_drm *d, char *err, int errlen) {
  if (d->cursor_fb_id && d->cursor_map) {
    return 0;
  }
  uint32_t w = d->cursor_max_w ? d->cursor_max_w : 256;
  uint32_t h = d->cursor_max_h ? d->cursor_max_h : 256;
  struct drm_mode_create_dumb create;
  memset(&create, 0, sizeof(create));
  create.width = w;
  create.height = h;
  create.bpp = 32;
  if (drmIoctl(d->fd, DRM_IOCTL_MODE_CREATE_DUMB, &create) != 0) {
    seterr(err, errlen, "cursor CREATE_DUMB", errno);
    return -1;
  }
  uint32_t fb_id = 0;
  if (drmModeAddFB(d->fd, w, h, 32, 32, create.pitch, create.handle, &fb_id) !=
      0) {
    struct drm_mode_destroy_dumb destroy = {.handle = create.handle};
    drmIoctl(d->fd, DRM_IOCTL_MODE_DESTROY_DUMB, &destroy);
    seterr(err, errlen, "cursor AddFB", errno);
    return -1;
  }
  struct drm_mode_map_dumb map;
  memset(&map, 0, sizeof(map));
  ...
  if (drmIoctl(d->fd, DRM_IOCTL_MODE_MAP_DUMB, &map) != 0) {
    ...
    struct drm_mode_destroy_dumb destroy = {.handle = create.handle};
    drmIoctl(d->fd, DRM_IOCTL_MODE_DESTROY_DUMB, &destroy);
    seterr(err, errlen, "cursor MAP_DUMB", errno);
    return -1;
  }
  void *ptr = mmap(NULL, create.size, PROT_READ | PROT_WRITE, MAP_SHARED, d->fd,
                   (off_t)map.offset);
  if (ptr == MAP_FAILED) {
    drmModeRmFB(d->fd, fb_id);
    struct drm_mode_destroy_dumb destroy = {.handle = create.handle};
    drmIoctl(d->fd, DRM_IOCTL_MODE_DESTROY_DUMB, &destroy);
    seterr(err, errlen, "cursor mmap", errno);
    return -1;
  }
  d->cursor_handle = create.handle;
  d->cursor_fb_id = fb_id;
  d->cursor_map = ptr;
  d->cursor_pitch = create.pitch;
  d->cursor_size = create.size;
  return 0;
}
...
int worldr_drm_cursor_argb(worldr_drm *d, int32_t x, int32_t y, uint32_t width,
  ...
  if (!d || d->fd < 0 || !bgra || width == 0 || height == 0) {
    seterr(err, errlen, "drm cursor: missing session or pixels", 0);
    return -1;
  }
  discover_planes(d);
  if (!d->cursor_id) {
    seterr(err, errlen, "drm cursor: no cursor plane", 0);
    return -1;
  }
  if (width > d->cursor_max_w || height > d->cursor_max_h) {
    seterr(err, errlen, "drm cursor: larger than plane cap", 0);
    return -1;
  }
  if (ensure_cursor_fb(d, err, errlen) != 0) {
    return -1;
  }
  memset(d->cursor_map, 0, d->cursor_size);
  uint8_t *dst = (uint8_t *)d->cursor_map;
  for (uint32_t row = 0; row < height; row++) {
    ...
    memcpy(dst + row * d->cursor_pitch, bgra + row * stride, n);
  }
  if (atomic_set_plane(d, d->cursor_id, d->cursor_fb_id, x, y, width, height,
                       width, height) != 0) {
    seterr(err, errlen, "cursor atomic commit", errno);
    return -1;
  }
  d->cursor_active = 1;
  return 0;
...
int worldr_drm_cursor_disable(worldr_drm *d, char *err, int errlen) {
  if (!d || d->fd < 0) {
    seterr(err, errlen, "drm cursor disable: missing session", 0);
    return -1;
  }
  if (!d->cursor_active) {
    return 0;
  ...
  discover_planes(d);
  if (d->cursor_id) {
    (void)atomic_set_plane(d, d->cursor_id, 0, 0, 0, 0, 0, 0, 0);
  }
  d->cursor_active = 0;
  return 0;
}
